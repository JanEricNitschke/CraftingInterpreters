...
impl<V: std::fmt::Debug + std::cmp::PartialEq + HasTypeName> Storage<V> {
    #[must_use]
    fn new() -> Self {
        Self {
            data: HashMap::new(),
...
impl<V: HasTypeName> std::ops::Index<usize> for Storage<V> {
    type Output = V;
...
impl<V: HasTypeName> std::ops::IndexMut<usize> for Storage<V> {
